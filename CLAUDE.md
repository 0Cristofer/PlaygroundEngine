...
Other named modules: `.App` (`AppBase`, `EngineContext`), `.Ecs` (umbrella, re-exports `.Ecs.Component`, `.Ecs.Entity`, `.Ecs.TransformComponent` and the `:System` partition), `.Ecs.InputSystem` and `.Ecs.InputSystem.InputStateComponent`, `.Ecs.CameraComponent` (imports `.Ecs`, so it is not re-exported by it), `.Math` (umbrella over the `:Types` and `:Transform` partitions), `.Renderer.View` (the `ExtractedView` seam and the world-to-view and projection builders; imports `.Math` alone) and `.Renderer.Frame` (the `ExtractedFrame` payload the renderer is handed), `.RenderExtraction` (L3, the only place that reads the simulation and writes render data), `.PlatformEvents`, `.WindowServer`, `.Renderer.Vulkan`, `.DebugUi` (re-exports `.DebugUi.Annotations`, which carries the `DrawDebug` tag with no dependencies so any band can annotate), `.Reflection` (umbrella over `.Reflection.Core` and `.Reflection.Builtins`), `.Log`, `.Diagnostics`, `.Files`, `.Paths`, `.Image`, `.Model`, `.FrameCapture`, `.AgentChannel`.
