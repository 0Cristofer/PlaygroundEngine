...
TEST_CASE("an argument with no object is rejected before it is bound")
{
	...
	int y = 2;
	const std::array args{PgE::TypedRef{.Type = &PgE::TypeOf<int>(), .Data = nullptr},
						  PgE::TypedRef{.Type = &PgE::TypeOf<int>(), .Data = &y}};
...
TEST_CASE("construction validates arity, argument type, and the destination slot")
{
	...
	int x = 1;
	int y = 2;
	std::array goodArgs{PgE::TypedRef{.Type = &PgE::TypeOf<int>(), .Data = &x},
										  PgE::TypedRef{.Type = &PgE::TypeOf<int>(), .Data = &y}};
	...
	double wrong = 3.0;
	std::array badArgs{PgE::TypedRef{.Type = &PgE::TypeOf<double>(), .Data = &wrong},
										 PgE::TypedRef{.Type = &PgE::TypeOf<int>(), .Data = &y}};
