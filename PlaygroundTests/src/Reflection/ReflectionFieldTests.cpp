...
TEST_CASE("erased field access moves values through a typed slot")
{
	...
	int readBack = 0;
	const auto got = type.GetFieldValue(PgE::TypedRefOf(person), "Age", {.Type = &PgE::TypeOf<int>(), .Data = &readBack, .IsConst = false});
	REQUIRE(got.has_value());
	CHECK(readBack == 40);

	int newAge = 41;
	const auto set = type.SetFieldValue(PgE::TypedRefOf(person), "Age", {.Type = &PgE::TypeOf<int>(), .Data = &newAge, .IsConst = true});
...
TEST_CASE("field access through a resolved FieldInfo")
{
	...
	int readBack = 0;
	REQUIRE(age->GetValue(PgE::TypedRefOf(person), {&PgE::TypeOf<int>(), &readBack, false}).has_value());
	CHECK(readBack == 30);

	int newAge = 31;
	REQUIRE(age->SetValue(PgE::TypedRefOf(person), {&PgE::TypeOf<int>(), &newAge, true}).has_value());
	CHECK(person.Age == 31);
}
...
TEST_CASE("borrowed field reference reads and writes in place")
{
	...
	const auto ref = type.GetFieldRef(PgE::TypedRefOf(person), "Age");
...
{
	Packet packet{};
	const auto ref = PgE::TypeOf<Packet>().GetFieldRef(PgE::TypedRefOf(packet), "Version");
...
TEST_CASE("borrow constness follows the object and the member")
{
	...
	const auto viaConstObject = type.GetFieldRef(PgE::TypedRefOf(std::as_const(fixed)), "Variable");
...
TEST_CASE("borrowed reference member assigns through to the referent")
{
	...
	const auto aliasRef = type.GetFieldRef(PgE::TypedRefOf(referencing), "Alias");
	...
	const auto constAlias = type.GetFieldRef(PgE::TypedRefOf(referencing), "ConstAlias");
...
TEST_CASE("non-copyable non-movable field is reachable only through the borrow")
{
	...
	const auto get = type.GetFieldValue(PgE::TypedRefOf(holder), "Item", {.Type = &PgE::TypeOf<Immovable>(), .Data = nullptr, .IsConst = false});
...
TEST_CASE("a value walk refuses to read a union member by member")
{
	Scalar scalar{};
	scalar.AsInt = 1065353216;

	// Reflection lists both members, which is the shape a serializer needs. Reading them in turn is the
	// part C++ does not license: only one is live, and nothing records which. Rendering states the type
	// rather than punning the bytes into every alternative.
	CHECK(PgE::TypeOf<Scalar>().GetFields().size() == 2);
	CHECK(PgE::ToString(scalar) == "<union>");
}

TEST_CASE("a const object does not make a mutable or reference member read-only")
{
	const Members members{};
	const Referencing referencing{};

	// Both writes are legal C++ through a const object: mutable is exempt by definition, and a reference
	// member's referent is a separate object the qualification never reached. The erased path follows.
	static_assert(requires(const Members& m) { m.Cached = 5; });
	static_assert(requires(const Referencing& r) { r.Alias = 5; });

	const PgE::FieldInfo* cached = PgE::TypeOf<Members>().FindFieldByIdentifier("Cached");
	REQUIRE(cached != nullptr);
	CHECK_FALSE(cached->GetRef(PgE::TypedRefOf(members))->IsConst);

	int replacement = 9;
	CHECK(cached->SetValue(PgE::TypedRefOf(members), PgE::TypedRefOf(replacement)).has_value());
	CHECK(members.Cached == 9);

	const PgE::FieldInfo* alias = PgE::TypeOf<Referencing>().FindFieldByIdentifier("Alias");
	REQUIRE(alias != nullptr);
	CHECK_FALSE(alias->GetRef(PgE::TypedRefOf(referencing))->IsConst);

	// A plain member of the same const object still is read-only, so the exemption is not a blanket one.
	const PgE::FieldInfo* plain = PgE::TypeOf<Members>().FindFieldByIdentifier("Plain");
	REQUIRE(plain != nullptr);
	CHECK(plain->GetRef(PgE::TypedRefOf(members))->IsConst);
	CHECK(plain->SetValue(PgE::TypedRefOf(members), PgE::TypedRefOf(replacement)).error().Reason == PgE::FieldError::ConstViolation);
}

TEST_CASE("the typed field sugar refuses a pointer and answers a null object")
{
	const PgE::FieldInfo* age = PgE::TypeOf<Person>().FindFieldByIdentifier("Age");
	REQUIRE(age != nullptr);

	// The object binds, its address does not: that stops an un-migrated call erasing as Person* and handing
	// back a valueless expected. Only the positive half is assertable, since GCC 16 hard-errors on a failed
	// constraint inside a requires instead of yielding false.
	static_assert(requires(const PgE::FieldInfo& field, Person& object) { field.GetAs<int>(object); });

	// A borrow with no object is its own answer: NotAddressable is the field's, and belongs to a bitfield.
	const PgE::TypedRef noObject{.Type = &PgE::TypeOf<Person>()};
	CHECK(age->GetRef(noObject).error().Reason == PgE::FieldError::NullObject);

	const PgE::FieldInfo* bitfield = PgE::TypeOf<Members>().FindFieldByIdentifier("Small");
	REQUIRE(bitfield != nullptr);
	Members members{};
	CHECK(bitfield->GetRef(PgE::TypedRefOf(members)).error().Reason == PgE::FieldError::NotAddressable);
}
