...
namespace ReflectionTestTypes
{
    ...
    struct MoveOnlyHolder { MoveOnlyValue Item; };
    ...
    struct ImmovableHolder { Immovable Item; };

    // A nested reflected struct plus a pointer member.
    struct Inner { int A = 1; };
    ...
    struct Palette { Shade Primary = Shade::Green; int Count = 2; };
...
TEST_CASE("field access through a resolved FieldInfo")
{
    ...
    const PgE::FieldInfo* age = PgE::TypeOf<Person>().FindFieldByName("Age");
    REQUIRE(age != nullptr);
    CHECK(age->GetName() == "Age");
...
TEST_CASE("FindFieldByName misses return nullptr")
{
    CHECK(PgE::TypeOf<Person>().FindFieldByName("Nope") == nullptr);
}
...
TEST_CASE("reference members read through to the referent and are not writable")
{
    ...
    const PgE::FieldInfo* alias = type.FindFieldByName("Alias");
...
{
    Consumer consumer{};
    const PgE::FuncInfo* store = PgE::TypeOf<Consumer>().FindFunctionsByName("Store").front();
...
TEST_CASE("invoke copies a by-value argument with a deleted move constructor")
{
    ...
    const PgE::FuncInfo* take = PgE::TypeOf<CopyConsumer>().FindFunctionsByName("Take").front();
...
TEST_CASE("invoking through a base type calls the overwritten function")
{
	...
	auto baseFunctions = baseType.FindFunctionsByName("GetV");
	auto childFunctions = childType.FindFunctionsByName("GetV");
	auto childNoOverrideFunctions = childNoOverrideType.FindFunctionsByName("GetV");
