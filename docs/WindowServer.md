...
Status: implemented, for the "now" rows of the ownership table. The boundary rule, the ownership split, and the event record's shape are settled. The interfaces below are what shipped; see [Implementation notes](#implementation-notes) for where they differ from the original sketch. Parent document: [ApplicationArchitecture.md](ApplicationArchitecture.md). Engine-wide conventions live in [CoreConventions.md](CoreConventions.md). The dev overlay that consumes this first is sketched in [DebugUISketch.md](DebugUISketch.md), parts of which this document overrides (see "Overrides").
...
**Backend partition.** Compile-time implementation selection. *Everything* platform-specific gets one: networking, file I/O, time, the window system. This is the existing pattern in `Window.cppm`, and it is what [ApplicationArchitecture.md](ApplicationArchitecture.md) mandates in place of runtime-polymorphic backends. It answers "this code differs per OS."
...
The record is a **per-pump immutable batch**. `Pump()` clears it, fills it, and returns; afterwards it is read-only for the rest of the frame. Consumers read a `std::span` and never drain it, so several can read the same batch.
...
**`Modifiers` are per-event**, despite looking derivable from key transitions. They are not: with `GLFW_LOCK_KEY_MODS` enabled, GLFW reports Caps Lock and Num Lock *lock state*, which no amount of watching presses reconstructs. That input mode is off by default and the backend must enable it at window creation, or the justification for this field is void.
...
`WindowServer` vends cohesive sub-facades rather than being one flat class: `Windows`, `Monitors`, `Clipboard`, `Cursor`, `TextInput`. Each has its own backend concept.

This is not cosmetic. The "later" rows in the ownership table are, item for item, the contents of Godot's `DisplayServer`. Compile-time backends make the god-object outcome *worse* than Godot's, because a concept with two hundred requirements is far harder to satisfy than a vtable when a second platform arrives. Per-facade concepts let a new backend be brought up one facade at a time.
...
public:
    ...
    // Clears the record, drains the OS queue into it, and returns. Main thread only.
    void Pump(PlatformEventRecord& record);

    // Fires window signals from the record. A drain point, called explicitly by the root,
    // never from inside an OS callback.
...
**Signals are owned by `WindowServer`**, not re-emitted by the root, per the architecture document's rule that signals belong to the emitting system. The root subscribes at wiring time and calls `DispatchWindowEvents` at its chosen point in the frame.
...
1. **The debug overlay adapter** walks the span in order and forwards to `io.AddKeyEvent`, `io.AddInputCharacter`, and friends. It needs the record and nothing else, which keeps the first slice small.
2. **`WindowServer` itself** dispatches window signals from it.
...
**Destruction order is renderer, then windows, then server.** The renderer holds a `vk::raii::SurfaceKHR` referencing a window; today this is preserved by member declaration order in `Engine`. It is the trap this refactor creates, because it becomes tempting for the server's destructor to destroy all windows.
...
```
PlaygroundEngine.WindowServer
  :events    PlatformEvent, PlatformEventRecord, InputCode, modifiers, key token
  :common    specifications, sizes, errors, backend forward declarations; re-exports :events
  :backend   CMake-selected; holds WindowServerBackend and WindowBackend together
```

`:events` is a split off `:common` rather than a third concept: the code space alone is long enough that keeping it in one file buried the specifications and errors beside it. `:common` re-exports it, so an importer sees one surface.

Both backends live in one partition so their cooperation stays an implementation detail with no exposed seam. This is what makes the "how do the window and the pump connect, given that the connection is backend-specific?" problem disappear: the server creates the windows, so its backend already holds whatever native handle it needs. The GLFW backend registers per-window callbacks that route into one buffer; a Wayland backend never touches a surface for input at all, reading the seat and attributing to the focused surface. Same contract, opposite internals.

Concept-checked, compile-time selected, no vtable, matching the existing pattern in `Window.cppm`.

## Implementation notes

Where the shipped code differs from the sketches above, and what the slice added that the design did not name.

- **Typed signals were built here.** [ApplicationArchitecture.md](ApplicationArchitecture.md) names them as one of the three interconnection mechanisms but nothing had implemented one, and `DispatchWindowEvents` is meaningless without them. `PlaygroundEngine.Signal` is the minimum that satisfies that document: `Signal<Payload...>` owned by the emitting system, `SignalSubscription` as a move-only RAII handle, no global bus. The subscriber owns a token and the slot only observes it, so a subscription outliving its signal and a signal outliving its subscriber are both safe.
- **Window creation lives on the `Windows` sub-facade**, not flat on the server as the interface sketch had it. The sketch predates the sub-facade decision; that decision wins.
- **`CloseRequested` reaches the root as a signal**, not as a record walk. Subscribing is what "the root remembers" means, and it keeps the record read out of the frame loop.
...
- **The window user pointer carries the server, not the window.** With no window identity field in the record, a callback needs nothing but the record the server is currently filling.
...
- **`InputModifiers` is a struct of `bool`s**, not a bitmask. Six bytes against one, in exchange for staying reflectable, printable and readable in a designated initializer without a flags facet that does not exist.
- **Timestamps are `steady_clock` nanoseconds read when the callback runs**, which is still "arrival at pump" precision: the callbacks all run inside `glfwPollEvents`.
